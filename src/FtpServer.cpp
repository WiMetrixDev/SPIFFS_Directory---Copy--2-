...
uint8_t FtpServer::handleFTP()
{
  ...
  if ((int32_t)(millisDelay - millis()) <= 0)
  {
        ...
        dataConn != dataConn0 || dstat != data0)
    {
      ...
      DEBUG_PRINT(F("  Data socket: "));
      DEBUG_PRINT(hex);
      DEBUG_PRINT(int(dstat));
      DEBUG_PRINTLN(dec);
    }
#endif
  }
...
void FtpServer::clientConnected()
{
  ...
  client.println(F("220---   By Naveed Ahmed   ---"));
  ...
  else if (CommandIs("STOR") || CommandIs("APPE"))
  {
    ...
    if (haveParameter() && makePath(path))
    {
      ...
      if (exists(path))
      {
        ...
        open = openFile(path, (CommandIs("APPE") ? O_APPEND : O_CREAT));
#else
...
{
  bool openD;
#if STORAGE_TYPE == STORAGE_LITTLEFS || STORAGE_TYPE == STORAGE_SD
  if (cwdName == 0)
  {
